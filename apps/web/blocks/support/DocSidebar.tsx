                    ...
                    <SidebarMenuItem>
                        ...
                        >
                            <div className="relative w-8 h-8 flex-shrink-0 transition-transform group-hover:scale-110 duration-300">
                                {theme === "dark" ?
                                    <Image src={documentation?.darkLogo} alt={documentation?.title} fill className="object-contain" unoptimized /> :
                                    <Image src={documentation?.logo} alt={documentation?.title} fill className="object-contain" unoptimized />}
                            </div>
                                ...
                                <SidebarMenu>
                                    ...
                                    {categoryDocs.map((doc) => (
                                        <SidebarMenuButton asChild tooltip={doc.Name} key={doc.id}
                                            className={cn("cursor-pointer text-xs", pathname === "/doc/" + doc.slug && "bg-sidebar-accent")}
                                            onClick={() => router.push("/doc/" + doc.slug as string)}>
